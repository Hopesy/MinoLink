...
### Added
- 更新弹窗支持“下载并更新”一键流转：下载 MSI 成功后自动关闭当前应用、安装完成后自动拉起新版本。

### Changed
- 自启按钮图标调整为更标准的电源圆环样式。
- 安装包发布链路收口为 GitHub Actions：推送 `v*` 版本 tag 后自动在远端构建 MSI、创建或更新 GitHub Release，并上传安装包。
- 删除本地 `publish-installer.ps1/.cmd` 发布脚本，避免本地/远端两套发布入口长期并存。
- README 的发布说明同步更新为远端自动发布流程，版本示例切到 `1.0.6`。
...
### Removed
- 移除仓库内已不再维护的测试工程、计划文档和本地发布脚本，减少发布相关噪音与维护面。
